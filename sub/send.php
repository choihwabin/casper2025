...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (!empty($_POST["mb_type"])) {
        $mb_type = htmlspecialchars($_POST["mb_type"]); // 보안 처리

        // 데이터 확인
        echo "선택한 문의 유형: " . $mb_type;

        // 추가적으로 DB 저장, 이메일 발송 등의 작업을 여기에 추가 가능
    } else {
        echo "문의 유형을 선택해주세요.";
    }
} else {
...
$select = strip_tags(htmlspecialchars($_POST["mb_type"]));
	
// 이메일을 생성하고 메일을 전송하는 부분
$to = 'chlghkqls123@naver.com'; // 받는 측의 이메일 주소를 기입하는 부분
